      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-gray-600 text-lg">
            Proxy servislerini yönetin, ekleyin ve düzenleyin
          </p>
        </div>
        ...
        <table class="w-full">
          <thead>
            <tr class="border-b border-gray-200">
              <th class="text-left py-4 px-6 font-semibold text-gray-900">Proxy Adı</th>
          ...
          <tbody>
            ...
            <tr v-else-if="filteredProxies.length === 0" class="border-b border-gray-100">
              <td colspan="4" class="py-8 text-center text-gray-500">
                <div class="flex flex-col items-center space-y-2">
                  <svg class="w-12 h-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9v-9m0-9v9" />
                  </svg>
                  <p>Henüz proxy bulunmuyor</p>
                  <button 
                    @click="openAddModal"
                    class="text-apple-blue hover:text-apple-blue/80 font-medium"
                  >
                    İlk proxy'yi ekleyin
                  </button>
                </div>
              </td>
            </tr>
            <tr v-else v-for="proxy in filteredProxies" :key="proxy.id" class="border-b border-gray-100 hover:bg-gray-50">
              <td class="py-4 px-6">
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    <p class="font-medium text-gray-900">{{ proxy.name }}</p>
                    <p class="text-sm text-gray-500">ID: {{ proxy.id }}</p>
                  </div>
                </div>
              </td>
              <td class="py-4 px-6">
                <span 
                  ...
                  :class="proxy.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'"
                >
                  <span class="w-1.5 h-1.5 rounded-full mr-1.5" :class="proxy.is_active ? 'bg-green-400' : 'bg-red-400'"></span>
                  {{ proxy.is_active ? 'Aktif' : 'Pasif' }}
                </span>
              </td>
              <td class="py-4 px-6">
                <p class="text-sm text-gray-500">{{ formatDate(proxy.created_at) }}</p>
              </td>
              <td class="py-4 px-6">
                <div class="flex items-center justify-end space-x-2">
                  <button 
                    @click="toggleProxyStatus(proxy)"
                    :class="proxy.is_active ? 'text-red-600 hover:text-red-700' : 'text-green-600 hover:text-green-700'"
                    class="p-2 transition-colors"
                    :title="proxy.is_active ? 'Pasifleştir' : 'Aktifleştir'"
                  >
                    <svg v-if="proxy.is_active" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="bg-white rounded-2xl p-6 w-full max-w-md mx-4">
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-lg font-semibold text-gray-900">
            {{ editingProxy ? 'Proxy Düzenle' : 'Yeni Proxy' }}
          </h3>
        ...
        <form @submit.prevent="saveProxy" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Proxy Adı.</label>
            <input 
              v-model="form.name"
              type="text" 
              required
              ...
              placeholder="Örn: Proxy-1, Main Proxy"
            />
          </div>
          ...
          <div class="flex space-x-3 pt-4">
            ...
            >
              ...
              <span v-else>{{ editingProxy ? 'Güncelle' : 'Ekle' }}</span>
            </button>
          </div>
        </form>
      </div>
...
const filteredProxies = computed(() => {
  ...
  return proxies.value.filter(proxy => 
    proxy.name.toLowerCase().includes(searchQuery.value.toLowerCase())
  )
})
...
const loadProxies = async () => {
  try {
    ...
    if (response.success) {
      ...
      showMessage('Proxy\'ler yüklenemedi.', 'error')
    }
  } catch (error) {
...
const saveProxy = async () => {
  try {
    ...
    if (editingProxy.value) {
      response = await $api.put(`/proxy-settings/${editingProxy.value.id}`, form.value)
    } else {
      response = await $api.post('/proxy-settings', form.value)
    }
    
    if (response.success) {
      showMessage(
        editingProxy.value ? 'Proxy güncellendi!' : 'Proxy eklendi!', 
        'success'
      )
      closeModal()
      loadProxies()
...
const deleteProxy = async (proxy) => {
  if (!confirm(`"${proxy.name}" proxy'sini silmek istediğinizden emin misiniz?`)) {
    return
  }
  ...
  try {
    loading.value = true
    const { $api } = useNuxtApp()
    const response = await $api.delete(`/proxy-settings/${proxy.id}`)
    
    if (response.success) {
      showMessage('Proxy silindi!', 'success')
      loadProxies()
    } else {
...
const toggleProxyStatus = async (proxy) => {
  try {
    loading.value = true
    const { $api } = useNuxtApp()
    const response = await $api.put(`/proxy-settings/${proxy.id}/toggle-status`)
    
    if (response.success) {
      showMessage(
        proxy.is_active ? 'Proxy pasifleştirildi!' : 'Proxy aktifleştirildi!', 
        'success'
      )
      loadProxies()
    } else {
      showMessage(response.message || 'Durum değiştirme başarısız.', 'error')
    }
  } catch (error) {
    console.error('Toggle proxy status error:', error)
    showMessage('Durum değiştirme başarısız.', 'error')
  } finally {
    loading.value = false
  }
}
...
const deleteAllProxies = async () => {
  if (!confirm(`Tüm proxy'leri silmek istediğinizden emin misiniz? Bu işlem geri alınamaz!`)) {
    return
  }
  ...
  try {
    loading.value = true
    const { $api } = useNuxtApp()
    const response = await $api.delete('/proxy-settings/delete-all')
    
    if (response.success) {
      showMessage(`Tüm proxy'ler silindi! (${response.deleted_count} adet)`, 'success')
      loadProxies()
    } else {
      showMessage(response.message || 'Silme işlemi başarısız.', 'error')
    }
